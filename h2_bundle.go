...
func (cc *http2ClientConn) encodeHeaders(req *Request, addGzipHeader bool, trailers string, contentLength int64) ([]byte, error) {
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		// Save header sorting idxs
		headerSorting := make(map[string]int)
		for idx, hdr := range req.Header.Values(HeaderOrderKey) {
			headerSorting[strings.ToLower(hdr)] = idx
		}

		// Create a slice that has enough items to hold all ordered headers. The rest will simply be appended
		sortedHeaders := make([]struct {
			Name   string
			Values []string
		}, len(req.Header.Values(HeaderOrderKey)))

		// Will hold all cookies so we can put them into place easier
		cookieHeaders := make([]struct {
			Name  string
			Value string
		}, 0)
		...
		for k, vv := range req.Header {
			...
			if k == strings.ToLower(HeaderOrderKey) {
				continue
			}
			...
			if strings.EqualFold(k, "host") || strings.EqualFold(k, "content-length") {
				...
				for _, v := range vv {
					for {
						...
						cookieHeaders = append(cookieHeaders, struct {
							Name  string
							Value string
						}{Name: "cookie", Value: v[:p]})
						p++
					...
					// also take care of the first cookie, if any
					if len(v) > 0 {
						cookieHeaders = append(cookieHeaders, struct {
							Name  string
							Value string
			...
			if idx, ok := headerSorting[k]; ok {
				sortedHeaders[idx] = struct {
					Name   string
					Values []string
				}{Name: k, Values: vv}
			} else {
		...
		*/
		...
		for _, vv := range sortedHeaders {
			for _, v := range vv.Values {
				f(vv.Name, v)
			}
		}
		for _, vv := range cookieHeaders {
			f("cookie", vv.Value)
		}
	}
